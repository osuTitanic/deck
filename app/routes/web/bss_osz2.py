...
def validate_upload_request(
    ...
    if not config.BEATMAP_SUBMISSION_ENABLED:
        ...
        return bss.error_response(5, 'The beatmap submission system is currently disabled. Please try again later!')
    ...
    if not user:
        return bss.error_response(5, 'Authentication failed. Please check your username and password and try again!')
    ...
    if beatmapset := bss.resolve_beatmapset(set_id, beatmap_ids, session):
        # User wants to update an existing beatmapset
        resolved_set_id = beatmapset.id

        allowed_beatmaps, can_update_resources = bss.beatmap_update_permissions(
            user,
            beatmapset,
            session=session
        )
        ...
        if not can_update_resources and len(beatmap_ids) != len(beatmapset.beatmaps):
            ...
            return bss.error_response(5, 'You are not allowed to add additional beatmaps to this beatmapset.')

        # Create/Remove new beatmaps if necessary
        updated_beatmap_ids = bss.update_beatmaps(
            user,
            beatmap_ids,
            beatmapset,
            session=session
        ...
        if remaining_beatmaps <= 0:
            ...
            return bss.error_response(5, "You have reached your maximum amount of beatmaps you can upload.")

        # Create a new empty beatmapset inside the database
        resolved_set_id, updated_beatmap_ids = bss.create_beatmapset(
            user,
            beatmap_ids,
            session=session
        )
        ...
        if resolved_set_id is None:
            return bss.error_response(5, "An error occurred while creating the beatmapset.")
    ...
    full_submit = bss.is_full_submit(resolved_set_id, osz2_hash)
...
def upload_beatmap(
    ...
    if not beatmapset:
        ...
        return bss.error_response(5, 'The beatmapset you are trying to upload to does not exist. Please try again!')
    ...
    if submission_file.size and submission_file.size > 100_000_000: # 100 MB
        ...
        return bss.error_response(5, 'Your beatmap is too big. Try to reduce its filesize and try again!')
    ...
    if not full_submit:
        ...
        if not current_osz2_file:
            ...
            return bss.error_response(5, 'The osz2 file is missing. Please try again!')
    ...
    if not osz2_file:
        ...
        return bss.error_response(5, 'Something went wrong while processing your beatmap. Please try again!')
    ...
    if beatmapset.creator_id != user.id:
        ...
        osz2.files = bss.adjust_files_for_collaboration(
            osz2.files,
            bss.existing_files(beatmapset.id),
            allowed_beatmaps,
            can_update_resources
        )

    # Check if the user is trying to upload someone else's beatmap
    if bss.duplicate_beatmap_files(beatmapset, osz2.files, session):
        app.session.logger.warning(f'Failed to upload beatmap: Duplicate beatmap files')
        return bss.error_response(5, 'It seems like one of your beatmaps was already uploaded by someone else. Please try again!')
    ...
    if not bss.validate_beatmap_owner(osz2.metadata, osz2.beatmaps, allowed_usernames) and not user.is_bat:
        app.session.logger.warning(f'Failed to upload beatmap: User does not own the beatmapset')
        return bss.error_response(1)
    ...
    if max_beatmap_length <= 1:
        ...
        return bss.error_response(5, 'Your beatmap is too short. Please try to make it longer and try again!')
    ...
    bss.update_beatmap_thumbnail(beatmapset, osz2.beatmaps, osz2.files)
    bss.update_beatmap_audio(beatmapset, osz2.beatmaps, osz2.files)
    bss.update_beatmap_files(osz2.files, session=session)
    ...
    # Update osz2 hashes
    bss.update_osz2_hashes(set_id, osz2, session)
